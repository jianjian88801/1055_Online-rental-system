<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="订单号" prop="dingdanhao">
          <el-input v-model="ruleForm.dingdanhao" 
              placeholder="订单号" clearable  :readonly="ro.dingdanhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="订单号" prop="dingdanhao">
              <el-input v-model="ruleForm.dingdanhao" 
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="租赁月长" prop="zulinyuezhang">
          <el-input v-model="ruleForm.zulinyuezhang" 
              placeholder="租赁月长" clearable  :readonly="ro.zulinyuezhang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="租赁月长" prop="zulinyuezhang">
              <el-input v-model="ruleForm.zulinyuezhang" 
                placeholder="租赁月长" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  computed: {
    jine:{
      get: function () {
        return 1*this.ruleForm.yuezujiage*this.ruleForm.zulinyuezhang
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.dizhi = true;
          ...
          if(o=='zulinyuezhang'){
            this.ruleForm.zulinyuezhang = obj[o];
	    this.ro.zulinyuezhang = true;
            continue;
          }
          if(o=='jine'){
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.dingdanzhuangtaiOptions = "发货中".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `fahuodingdan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fahuodingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fahuodingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
